...
public class PerformanceDashboardService {
    ...
    public PerformanceDashboardResponse getDashboard(Long employeeId) {
        ...
        List<String> trendLabels = trendPoints.stream().map(PerformanceViewMapper.PerformanceTrendPoint::monthLabel).toList();
        List<Integer> trendScores = trendPoints.stream().map(point -> nvl(point.score())).toList();
        List<PerformanceFeedbackResponse> feedbacks = performanceViewMapper.findDashboardFeedbacks(employeeId);
