...
public class OrganizationService {
    ...
    public OrganizationMemberDetailResponseDTO getOrganizationMemberDetail(Long viewerEmployeeId, Long targetEmployeeId) {
        ...
        OrganizationMemberDetailResponseDTO.PersonalInfo personalInfo = new OrganizationMemberDetailResponseDTO.PersonalInfo(
        ...
        OrganizationMemberDetailResponseDTO.HrInfo hrInfo = new OrganizationMemberDetailResponseDTO.HrInfo(
                hrInfoRow.orgName(),
        ...
        List<OrganizationMemberDetailResponseDTO.SkillItem> skills = skillRows.stream()
                .map(row -> new OrganizationMemberDetailResponseDTO.SkillItem(
                        row.skillId(),
                        row.category(),
                        row.skillName(),
                        row.acquisitionDate(),
        ...
        List<OrganizationMemberDetailResponseDTO.CareerItem> careers = careerRows.stream()
                .map(row -> new OrganizationMemberDetailResponseDTO.CareerItem(
                        row.careerId(),
                        row.companyName(),
                        row.orgName(),
                        row.startDate(),
    ...
    public EvidenceFileResponseDTO getOrganizationMemberSkillEvidence(Long viewerEmployeeId, Long targetEmployeeId, Long skillId) {
        validateSameTeamAccess(viewerEmployeeId, targetEmployeeId);
        HrFileRow fileRow = myPageMapper.findSkillFileByIdAndEmployeeId(targetEmployeeId, skillId)
                .orElseThrow(() -> new NotFoundException("EVIDENCE_NOT_FOUND", "증빙 파일을 찾을 수 없습니다."));

        return new EvidenceFileResponseDTO(fileRow.getHrFileId(), fileRow.getFileTitle(), fileRow.getFileUrl());
    }

    public EvidenceFileResponseDTO getOrganizationMemberCareerEvidence(Long viewerEmployeeId, Long targetEmployeeId, Long careerId) {
        validateSameTeamAccess(viewerEmployeeId, targetEmployeeId);
        HrFileRow fileRow = myPageMapper.findCareerFileByIdAndEmployeeId(targetEmployeeId, careerId)
                .orElseThrow(() -> new NotFoundException("EVIDENCE_NOT_FOUND", "증빙 파일을 찾을 수 없습니다."));
