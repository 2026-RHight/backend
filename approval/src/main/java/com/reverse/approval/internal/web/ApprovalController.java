...
public class ApprovalController implements ApprovalResource {
    ...
    public ResponseEntity<ApiResponse<String>> draftApproval(@RequestPart(value = "dto") DraftApproval dto,
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.success(approvalService.draftApproval(dto, files, user.getEmployeeId(), ApprovalStatus.PENDING)));
    }
    ...
    public ResponseEntity<ApiResponse<String>> tempApproval(@RequestPart(value = "dto") DraftApproval dto,
                                                            ...
                                                            @AuthenticationPrincipal CustomUser user) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.success(approvalService.draftApproval(dto, files, user.getEmployeeId(), ApprovalStatus.TEMP)));
    }

    @Override
    @GetMapping("/{approvalId}/attachments/{fileId}/download")
    @SecurityRequirement(name="JWT")
    public ResponseEntity<byte[]> downloadAttachment(@PathVariable("approvalId") Long approvalId,
    ...
    @SecurityRequirement(name = "JWT")
    public ResponseEntity<?> deleteApproval(@PathVariable("approvalId") Long approvalId,
                                            @AuthenticationPrincipal CustomUser user) {
