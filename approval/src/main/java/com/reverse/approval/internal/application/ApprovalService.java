...
public class ApprovalService implements ApprovalFacade {
    ...
    public String draftApproval(DraftApproval dto, List<MultipartFile> files, Long employeeId, ApprovalStatus status) {
        // HR 모듈에서 기안자(사원) 정보 조회
        EmployeeProfileDTO drafterProfile = hrFacade.getEmployeeProfile(employeeId);

        ElectronicApprovalParam approval = ElectronicApprovalParam.from(dto, drafterProfile, status);
        ...
        insertReferenceAndRecipientLines(dto.getReferenceLine(),dto.getReceipientLine(), approval.getApprovalId());
    ...
    public DownloadedApprovalFile downloadAttachment(Long approvalId, Long fileId) {
        ApprovalAttachmentRow attachment = approvalAttachmentMapper.findAttachmentByFileIdAndApprovalId(fileId, approvalId)
                .orElseThrow(() -> new AttachmentNotFoundException(
                        "첨부파일을 찾을 수 없습니다. approvalId=" + approvalId + ", fileId=" + fileId
                ));
    ...
    public void deleteApproval(Long approvalId, Long employeeId) {
        ...
        List<ApprovalAttachmentRow> attachments = approvalAttachmentMapper.findAttachmentsByApprovalId(approvalId);
        attachments.forEach(attachment -> approvalFileService.deleteByFileUrl(attachment.filePath()));
    ...
    private void insertDetailByDocType(DraftApproval dto, Long approvalId) {
        switch (dto.getDocType()) {
            ...
            case RTW -> {
                ...
                rtwMapper.insertRtwDetail(
                        RTWDetailParam.from(dto.getRtwRequest(), approvalId)
                );
            }
        }
    }

    private void insertApprovalLines(List<ApprovalLineRequest> approvalLineDto, Long approvalId, ApprovalStatus status) {
        if (approvalLineDto != null) {
            approvalLineDto.forEach(line -> {
                EmployeeProfileDTO profile = hrFacade.getEmployeeProfile(line.getApproverId());
    ...
    private void insertReferenceAndRecipientLines(List<ReferenceLineRequest> ref_dto, List<RecipientLineRequest> rec_dto, Long approvalId) {
        if (ref_dto != null) {
            ref_dto.forEach(line -> {
                EmployeeProfileDTO profile = hrFacade.getEmployeeProfile(line.getReferencerId());
                referenceLineMapper.insertReferenceLine(
                        ReferenceLineParam.from(
                                line.getReferencerId(),
                                approvalId,
                                profile.employeeName() == null ? "미지정" : profile.employeeName(),
        ...
        if (rec_dto != null) {
            rec_dto.forEach(line -> {
                EmployeeProfileDTO profile = hrFacade.getEmployeeProfile(line.getReceipientId());
